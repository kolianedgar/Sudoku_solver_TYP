...
def save_digit_image(digit_img, index, folder_name):
    """Saves the digit image to the specified folder with the given index."""
    create_subfolder(folder_name)  # Ensure the folder exists
    filename = f"{folder_name}/digit_{index + 1}.png"
...
def get_digits(img, squares, size, output_folder):
    """Extracts digits from their cells and saves them."""
    digits = []
    img = pre_process_image(img.copy(), skip_dilate=True)
    for i, square in enumerate(squares):
        digit_img = extract_digit(img, square, size)
        digits.append(digit_img)
        save_digit_image(digit_img, i, output_folder)  # Save each digit image
    return digits
...
def parse_grid(path):
    ...
    digits = get_digits(cropped, squares, 28, "sudoku_digits")  # Save digits to 'sudoku_digits' folder
...
# Source: https://github.com/Soumyajeet-Muni/Image-based-sudoku-Solver/blob/main/SudokuExtractor.py